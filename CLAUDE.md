...
- **`Merging/PriorityMerger`** — for each canonical ID, picks fields according to per-field source-order lists in `MergeOptions` (`PricingOrder`, `ContextOrder`, `CapabilitiesOrder`, `DisplayOrder`). Both `Pricing` and `Capabilities` merge by first-non-null-per-field (not whole-record pick), so a source that only has `BatchDiscountFraction` can still contribute it even when another source wins `InputCostPerMillion`. The tiered-pricing triple (`InputCostPerMillionAboveContextThreshold` / `OutputCostPerMillionAboveContextThreshold` / `ContextThresholdTokens`) is merged as a single unit to keep the threshold semantically paired with its rates. Configurable at `ModelRegistry:Merge`.
...
- **DTOs** in `Dtos/` (`ModelInfo`, `Pricing`, `Context`, `Capabilities`, `Modality`, `SourceState`, `CatalogMeta`) are **shared with the service** via project reference — changing a DTO changes the wire contract for every consumer. Treat as a public API. When adding fields to an existing record, make them nullable with a default value so positional constructor callers don't break, and bump the NuGet `<Version>` at minimum at the minor (pre-1.0) or major (post-1.0) level — record deconstruction and serialization shape both change even though the class-level API stays source-compatible.
- `Pricing` covers per-million-token rates for: base input/output, cache reads, reasoning output, cache writes (5-min and 1-hour Anthropic TTL), tiered long-context (input/output above `ContextThresholdTokens`, typically 200k for Gemini 2.5 Pro), and `BatchDiscountFraction` (scalar in [0,1], typically 0.5 for OpenAI/Anthropic/Gemini async batch). All new fields default to `null` when the upstream doesn't supply them. LiteLLM is the richest source; OpenRouter supplies cache-write; models.dev supplies context-over-200k input-side only.

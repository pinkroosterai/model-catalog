...
namespace ModelCatalog.Client.Tests;
...
public class ModelCatalogClientTests
{
    ...
    ) Build(ModelCatalogClientOptions? opts = null)
    {
        ...
        var client = new ModelCatalogClient(
            http,
            cache,
            Options.Create(opts),
            TimeProvider.System,
    ...
    public async Task GetModel_HitsNetworkThenCache()
    {
        ...
        var req = handler
            .When("http://fake/v1/models/openai/gpt-5")
            .Respond(JsonContent.Create(Sample()));
    ...
    public async Task GetModel_ReturnsStaleCache_WhenServiceUnreachable()
    {
        ...
        await cache.SetStringAsync(
            "modelregistry:v1:model:openai/gpt-5",

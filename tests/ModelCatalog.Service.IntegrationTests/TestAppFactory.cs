...
{
    private readonly string _snapshotPath = Path.Combine(Path.GetTempPath(), $"snap-{Guid.NewGuid()}.json");
    ...
    public static SourceSnapshot Snap(string source, params (string id, decimal? price, long? ctx)[] models)
    {
        ...
        return new SourceSnapshot(source, DateTimeOffset.UnixEpoch, models.Select(m => new ModelInfo(

...
{
    public static RouteGroupBuilder MapSourceEndpoints(
    ...
    )
    {
        ...
        g.MapGet(
            ...
            () =>
            {
                ...
                return Results.Ok(
                    store.Current.RawSources.Select(s => new
                    {
                        s.SourceName,
                        s.FetchedAt,
        ...
        g.MapGet(
            "/sources/{source}/models/{provider}/{modelId}",
            (string source, string provider, string modelId) =>
            {
                if (store.Current is null)
                    return Results.Problem(statusCode: 503);
...
#pragma warning restore CA1308
                var m = raw.Models.FirstOrDefault(x =>

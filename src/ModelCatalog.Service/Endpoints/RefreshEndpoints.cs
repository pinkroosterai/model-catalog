...
public static class RefreshEndpoints
{
        ...
        IOptionsMonitor<ApiKeyOptions> apiKeyOptions)
    {
        ...
        g.MapPost("/refresh", (HttpContext ctx) =>
        {
            var configured = apiKeyOptions.CurrentValue.ApiKeys;
            if (configured.Count == 0)
                return Results.Problem(statusCode: 503, detail: "Refresh disabled — no api keys configured");

            if (!ctx.Request.Headers.TryGetValue("X-Api-Key", out var provided)
                || !configured.Any(k => string.Equals(k.Key, provided.ToString(), StringComparison.Ordinal)))
            {

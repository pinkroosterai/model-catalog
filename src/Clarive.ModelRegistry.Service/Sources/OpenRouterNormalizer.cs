...
public sealed class OpenRouterNormalizer
{
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Performance", "CA1822", Justification = "Instance method for DI parity with other sources")]
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Minor Code Smell", "S2325", Justification = "Instance method for DI parity with other sources")]
    ...
    public SourceSnapshot Normalize(string rawJson, DateTimeOffset fetchedAt)
    {
        ...
        foreach (var el in data.EnumerateArray())
        {
            ...
            var inputModalities = el.TryGetProperty("architecture", out var arch)
                ...
                ? im.EnumerateArray().Select(x => x.GetString() ?? string.Empty)
                    .ToHashSet(StringComparer.OrdinalIgnoreCase)
                : new HashSet<string>(StringComparer.OrdinalIgnoreCase);
